...
Both front ends now send the same query — cues, embedded cover art and playlist
membership all ride along as extra columns the COPY sink picks up by name. What
still differs is drive hygiene, which the UI has no way to do:

|  | `ui/duckbox-usb` | UI **Build USB** |
|---|---|---|
| track selection | whole library, or a `WHERE` clause | only the ticked rows |
| cues / artwork / playlists | yes | yes |
...
**If you export from the UI, run `sync` yourself before ejecting.**

### Playlists come along whole

A track carries *every* playlist it belongs to, so exporting a subset can put
partial copies of other playlists on the drive. Exporting the 8 tracks of
`popoo` wrote two playlists — `popoo` with its 8 entries, and `hi` with the one
track the two share. That is the same thing rekordbox does, and it is rarely
what surprises people twice, but it is worth knowing the first time.
